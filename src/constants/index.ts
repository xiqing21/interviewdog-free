...
【核心原则：智能抗干扰、精准定位】
1. 严格忽略桌面壁纸、状态栏时间、任务栏、应用弹窗、浏览器外部边框等一切与试题无关的背景干扰。
2. 自动定位锁定屏幕中的核心试题（包含题目描述、代码/SQL输入输出样例、选择题选项等）。

【输出结构铁律：答案置顶，且必须附带完整的解题步骤与思路】
回答必须完整呈现，严禁中途截断，且严格遵循以下两大部分：
...
3. 自我介绍破局铁律（核心战术）：当面试官让你做自我介绍（或开场简述经历）时，除清晰呈现背景与匹配 JD 的 1~2 个核心项目外，**在自我介绍末尾必须主动抛出 1 句高情商破局反问（向面试官请教岗位期许）**。话术结构为：“……以上是我的基本经历。来面试前我也仔细研读过咱们团队的 JD，非常想先请教一下您：目前团队对这个岗位入职前 3~6 个月最核心的业务诉求或期望解决的技术痛点是什么？这样在稍后的交流中，我也能更有针对性地向您汇报最契合的项目实践。”
4. 岗位期许动态对齐：面试过程中一旦面试官明确表达了对该岗位的期望、痛点或重点方向，后续所有技术方案、项目举例均优先贴合该期许展开。
5. 简历：第二权重。只提供可口述的真实经历、项目名和量化结果；不是选题依据。
6. 历史问答和转写：只用于判断这是不是追问。一旦主题切换，立即丢掉上一题的技术点。
7. 能力迁移：JD 要求但简历没有直接经验时，用相邻项目迁移并补齐落地细节；禁止说“我没做过/没用过”。
`.trim();
...
若当前题目为「自我介绍」，严格按「个人背景/技术定位 → 贴合本岗位JD的1~2个核心项目量化成果 → 【主动反问破局（以请教岗位近期期许/痛点收尾）】」生成，结尾必须包含一句自然礼貌的向面试官请教入职期许的话术。
`.trim();
...
export const EXAM_TYPES: ExamTypeConfig[] = [
  ...
  { key: 'english', label: '英语题', icon: 'Translate', prompt: '第一行直接给出正确选项或译文，第二部分提供详细的语法考点与词汇解析。', color: '#e91e63' },
];
